...
Run these five steps **in order**. Run each command **by itself**—do not redirect output or pipe one command into another. Stop at the first failure and report the error.
...
2. **Actually remove or use them:**
   ...
   - If it _should_ be used: add the proper usage (e.g. pass it through, log it, or use it in logic).
...
This uses `scripts/check-coverage-threshold.ts` to verify coverage thresholds. If this fails, improve tests or adjust the coverage configuration as appropriate before proceeding.
...
| 3    | `bun test`              | Yes             |
| 4    | `bun run test:coverage` | Yes             |
| 5    | `bun run test:e2e`      | Yes             |

Only report success when all five steps complete without errors.
