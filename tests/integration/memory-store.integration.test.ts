...
describe("MemoryStore with real SQLite (integration)", () => {
  ...
  it("should find memories matching a text query", async () => {
    const { store } = createStore();
    await store.store({ text: "User likes TypeScript and Bun", category: "preference", importance: 0.9 });
    await store.store({ text: "The weather is sunny today", category: "fact", importance: 0.3 });
    await store.store({ text: "TypeScript has great type inference", category: "fact", importance: 0.7 });
  ...
  it("should filter search by category", async () => {
    const { store } = createStore();
    await store.store({ text: "User likes dark mode", category: "preference", importance: 0.8 });
    await store.store({ text: "Dark mode saves battery", category: "fact", importance: 0.5 });
  ...
  it("should respect limit in search", async () => {
    const { store } = createStore();
    for (let i = 0; i < 10; i++) {
      await store.store({ text: `Memory item number ${i}`, category: "fact", importance: 0.5 });
    }
  ...
  it("should remove a memory by id", async () => {
    const { store } = createStore();
    const entry = await store.store({ text: "To be deleted", category: "other", importance: 0.1 });
    expect(await store.count()).toBe(1);
  ...
  it("should return search results ordered by importance descending", async () => {
    const { store } = createStore();
    await store.store({ text: "Low importance item", category: "fact", importance: 0.2 });
    await store.store({ text: "High importance item", category: "fact", importance: 0.9 });
    await store.store({ text: "Medium importance item", category: "fact", importance: 0.5 });
