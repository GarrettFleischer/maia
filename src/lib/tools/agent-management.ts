...
import type { AgentDefinition, AgentCreateConfig } from "../types";
...
export const agentCreateTool = makeTool(
  ...
  async (args) => {
    ...
    fs.writeFileSync(path.join(agentDir, "SOUL.md"), args.soul ?? `# ${args.name}\n\nI am ${args.name}, a Maia agent.\n`);
    fs.writeFileSync(path.join(agentDir, "MEMORY.md"), args.memory ?? "# Memory\n\nNo memories yet.\n");
    fs.writeFileSync(path.join(agentDir, "GOALS.md"), args.goals ?? "# Goals\n\n## Current Tasks\n- [ ] Awaiting instructions\n");
    fs.writeFileSync(path.join(agentDir, "USER.md"), args.user ?? "# User\n\nNo user information yet.\n");
...
export const agentDeleteTool = makeTool(
  ...
  async ({ agentId }) => {
    const db = getDb();
    db.prepare("UPDATE agents SET status = 'deleted', updated_at = ? WHERE id = ?")
...
export const agentListTool = makeTool(
  ...
  async () => {
    const db = getDb();
    return (db.prepare("SELECT * FROM agents WHERE status != 'deleted' ORDER BY created_at").all() as Record<string, unknown>[]).map(rowToAgent);
  }
);
...
export const agentGetTool = makeTool(
  ...
  async ({ agentId }) => {
    ...
    const read = (file: string) => {
      try { return fs.readFileSync(path.join(agentDir, file), "utf8"); } catch { return ""; }
    };
