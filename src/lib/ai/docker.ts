/**
 * @fileoverview Docker AI provider — OpenAI-compatible chat completions for Docker-hosted model servers.
 * @module lib/ai/docker
 */
...
/**
 * Docker provider for OpenAI-compatible servers running in containers (e.g. vLLM, TGI).
 * Uses /v1/chat/completions with SSE streaming; no API key by default.
 *
 * @brief Provider that calls a Docker-hosted server's OpenAI-compatible chat completions endpoint.
 */
export class DockerProvider implements AIProvider {
  private model: string;
  private baseUrl: string;
  private http: HttpClient;

  /**
   * @param model - Model identifier; "docker/" prefix is stripped before sending.
  ...
  constructor(model: string, baseUrl: string, http: HttpClient) {
    this.model = model.replace(/^docker\//, "");
    this.baseUrl = baseUrl.replace(/\/$/, "");
    this.http = http;
  }

  /**
   * @brief Stream a completion from the Docker-hosted server and return content and tool calls.
  ...
  async complete(
    ...
    if (!resp.ok) {
      ...
      throw new Error(`Docker error ${resp.status}: ${text}`);
    }
