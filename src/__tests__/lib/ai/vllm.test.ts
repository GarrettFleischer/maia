/**
 * @fileoverview Tests for vLLM AI provider (OpenAI-compatible SSE streaming, tool calls).
 * @module __tests__/lib/ai/vllm.test
 */
...
describe("VllmProvider", () => {
  ...
  it("throws on HTTP error with status and body", async () => {
    ...
    ).rejects.toThrow("vLLM error 503: Service unavailable");
  });

  it("strips vllm/ prefix from model in request body", async () => {
    let model = "";
