  ...
  describe("createOllamaEmbeddingAdapter", () => {
    ...
    it("accepts response with singular embedding array", async () => {
      ...
      http.on(
        "/api/embed",
        async () =>
          new FakeResponse(200, JSON.stringify({ embedding: vector })),
      );
    ...
    it("on generic 500, error does not add pull hint", async () => {
      ...
      await expect(adapter.embed("hello")).rejects.toThrow(
        /Ollama embeddings failed \(500\)/,
      );
      await expect(adapter.embed("hello")).rejects.not.toThrow(
        /Run: ollama pull/,
      );
    });
  });
    ...
    it("returns character budget from num_ctx in parameters (tokens * 2)", async () => {
      http.on(
        ...
        async () =>
          new FakeResponse(
            200,
            JSON.stringify({ parameters: "temperature 0.1\nnum_ctx 2048\n" }),
          ),
      );
      const chars = await getOllamaEmbedContextLength(
        "nomic-embed-text",
        "http://localhost:11434",
        http,
      );
    ...
    it("uses fallback when parameters has no num_ctx", async () => {
      http.on(
        ...
        async () =>
          new FakeResponse(
            200,
            JSON.stringify({ parameters: "temperature 0.7" }),
          ),
      );
      const chars = await getOllamaEmbedContextLength(
        "nomic-embed-text",
    ...
    it("caches result per model so show is not called twice", async () => {
      ...
      http.on("/api/show", async () => {
        ...
        return new FakeResponse(
          200,
          JSON.stringify({ parameters: "num_ctx 4096" }),
        );
      });
      const a = await getOllamaEmbedContextLength(
        "nomic-embed-text",
        "http://localhost:11434",
        http,
      );
      const b = await getOllamaEmbedContextLength(
        "nomic-embed-text",
        "http://localhost:11434",
        http,
      );
  ...
  describe("createEmbeddingAdapter", () => {
    it("throws when embedding model is not in whitelist", () => {
      ...
      http.on(
        "/api/embed",
        async () =>
          new FakeResponse(200, JSON.stringify({ embeddings: [[0.1]] })),
      );
      ...
      expect(() => createEmbeddingAdapter(settings, http)).toThrow(
        /not whitelisted/,
      );
    });
    ...
    it("creates adapter when embedding model is in whitelist", async () => {
      ...
      http.on(
        "/api/embed",
        async () =>
          new FakeResponse(200, JSON.stringify({ embeddings: [vector] })),
      );
      const settings: Settings = {
