...
describe("webSearchTool", () => {
  ...
  it("throws when BRAVE_SEARCH_API_KEY is not set", async () => {
    ...
    await expect(webSearchTool.execute({ q: "test" }, ctx)).rejects.toThrow("BRAVE_SEARCH_API_KEY");
  });
  ...
  it("returns parsed search results from Brave API", async () => {
    ...
    const ctx = makeToolCtx(http);
    const results = await webSearchTool.execute({ q: "example sites" }, ctx) as SearchResult[];
  ...
  it("respects maxResults limit", async () => {
    ...
    const ctx = makeToolCtx(http);
    const results = await webSearchTool.execute({ q: "test", max: 2 }, ctx) as SearchResult[];
  ...
  it("filters injection patterns in snippets", async () => {
    ...
    http.on("api.search.brave.com", async () => new FakeResponse(200, JSON.stringify(braveJson)));
    const ctx = makeToolCtx(http);
    const results = await webSearchTool.execute({ q: "test" }, ctx) as SearchResult[];
  ...
  it("throws when HTTP request fails", async () => {
    const ctx = makeToolCtx(http);
    await expect(webSearchTool.execute({ q: "anything" }, ctx)).rejects.toThrow();
  });

  it("throws when response is not ok", async () => {
    http.on("api.search.brave.com", async () => new FakeResponse(403, "Forbidden"));
    const ctx = makeToolCtx(http);
    await expect(webSearchTool.execute({ q: "test" }, ctx)).rejects.toThrow();
  });
