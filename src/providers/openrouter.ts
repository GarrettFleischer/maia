/**
 * @fileoverview OpenRouter LLM provider adapter.
 * @module providers/openrouter
 *
 * @brief Implements LLMProvider for OpenRouter API (OpenAI-compatible).
 * Supports chat streaming, model listing, and health checks. Requires API key
...
/** @brief Dependencies for createOpenRouterProvider */
...
const OPENROUTER_CONTEXT_SIZE = 4096;
 ...
 * @param acc - Map index -> AccumulatedToolCall
 * @param deltaToolCalls - choices[0].delta.tool_calls from SSE
 */
...
/**
 * @brief Creates an OpenRouter LLM provider.
 * @param deps - Dependencies: http, credentials, logger, credentialName, optional model
 * @returns LLMProvider implementation for OpenRouter
 */
export function createOpenRouterProvider(deps: OpenRouterProviderDeps) {
  ...
  return {
      ...
      try {
        const res = await http.fetch(url, {
          ...
          headers: {
            ...
            "HTTP-Referer": "https://github.com/maia-ai",
          },
          body,
        ...
        if (!res.ok) {
          throw new Error(
            `OpenRouter API error: ${res.status} ${res.body}`
          );
        }
        ...
        logger.error("OpenRouter chat failed", { error: msg });
    ...
    async listModels(): Promise<ModelInfo[]> {
      ...
      try {
        ...
        if (!res.ok) {
          throw new Error(
            `OpenRouter list models error: ${res.status} ${res.body}`
          );
        }
        ...
        return models.map((m) => ({
          id: m.id,
          name: m.name ?? m.id,
          contextWindow: m.context_length ?? OPENROUTER_CONTEXT_SIZE,
        }));
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        logger.error("OpenRouter listModels failed", { error: msg });
        throw new ProviderError(id, msg);
      }
    },
    ...
    async healthCheck(): Promise<boolean> {
      try {
        ...
        const msg = err instanceof Error ? err.message : String(err);
        logger.warn("OpenRouter health check failed", { error: msg });
        return false;
      }
    },
