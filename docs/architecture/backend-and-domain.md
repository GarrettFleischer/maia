...
**Smart context and prompt:** The agent system prompt no longer includes the last N conversation rounds verbatim; relevant prior context is supplied via the smart context pipeline. Query extraction uses only the **clarified commands** for the chat (not full round content); the agent can use **find_tool** to discover the tool for reading specific round numbers (e.g. `rounds: [1, 2]`) when it needs full context for those rounds. Smart context progress is streamed as `smart_context_phase` events (queries, retrieval, filter, done) so the client can show live status (e.g. "Extracting queries…", "Done (5 sources)"). Filtered sources, skills, and tools are included in full (no summarization step). The **done** phase output includes sources, active skills, **relevant tools**, and the **full prompt** sent to the main LLM: for every tool whose name appears in the matched skills content, the runner adds that tool’s name, description, and parameters to the done output so the agent and the done bubble have correct usage details (e.g. `cron_schedule` args as object, not string). The run (phases and outputs) is persisted as a single object on the session row (`sessions.smart_context_run`, `sessions.smart_context_after_message_index`), updated in place as each phase completes, so the UI can restore all phase bubbles on refresh.
...
**Rounds and reasoning:** The agent discovers tools via **find_tool** and skills via **find_skill**; only these two are in the minimal tool set sent to the LLM. The tool for reading rounds (discovered via find_tool) requires specific 1-based round number(s) (e.g. `rounds: [1, 2]`); it does not take "last N rounds". By default reasoning/thinking entries are omitted; the agent can pass `include_reasoning: true` to include the agent's reasoning for those rounds. When reasoning is included, only the **most recent thinking entry per round** (or per formatted slice) is included in the context so the context window is not filled by reasoning tokens; earlier thinking bubbles in the same round are omitted.
